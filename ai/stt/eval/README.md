...
- 잡음 폭: 설정을 안 바꿔도 생기는 흔들림. 로컬은 트랙에 들리지 않는 크기(16비트 1단계)의 잡음을 씨앗 셋으로 넣은 실행의 오류 글자 범위, Elice 는 같은 설정을 세 번 보낸 범위다
...
`--golden-root` 아래에서 아래 세 가지가 있는 폴더를 전부 회의로 읽는다.
...
- `meeting-02-aligned` 의 유재환 첫 발화 끝("저는 마지막에 정리하겠습니다. 그럼 시작할까요.")은 정렬본에서 마지막 발화 자리(129초 부근)에 들어가 있다. 유재환은 대본의 두 턴을 한 번에 읽었는데, 정렬본은 대본의 7발화판(`truth_utterances.json.orig`)으로 만들어 그 말을 두 자리로 나눠야 했다. `align` 은 이때 가장 긴 내부 쉼에서 나누고, m02 에서는 그 쉼이 "공유하고," 뒤였다. `RUN_GAP_S` 를 1~8초로 바꿔 다시 정렬해도 wav 가 같아서(2026-09-26 확인) 이 상수 탓이 아니다. 화자별 CER 은 시간순으로 이어 붙이므로 영향이 없고, 그 발화의 시작 시각·잃은 발화 판정은 이 자리 기준으로 읽는다
