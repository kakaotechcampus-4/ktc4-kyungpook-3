...
- 상태: 결정됨 (2026-09-16). 캡처는 `/record` 하나로 합쳤다. sink 는 StreamingSink + TrackWriter 이고
  ...
  2026-09-26 갱신: 실시간 경로는 지우지 않고, 이후 회의 중 전사를 다시 넣을 때 쓰려고 폴더로 보관한다(아래 다시 볼 조건)
...
- `.env.example` 주석: 두 Cog 를 함께 붙일 때 MESSAGE CONTENT 를 켜야 한다는 안내
...
9/16 에 배치를 운영 경로로 골랐다(#45 1차 리뷰). 처음에는 고르지 않은 쪽을 지우기로 했지만, 2026-09-26 에
실시간 경로는 지우지 않고 보관하기로 바꿨다. 이후 회의 중 전사를 서비스에 다시 넣을 수 있고, 그때 실제
서버에서 끝까지 돌았던 이 코드가 출발점이 된다(#45 1차 리뷰 고민 7 과도 맞는다).

보관하는 동안 실시간 경로는 운영 봇에 올리지 않는다(`tests/capture/test_operating_boundary.py`).
재연결 키 갱신처럼 두 경로에 다 필요한 수신 처리는 공용 계층(`capture/voice_client.py` 등)에만 둔다.
회의 중 전사를 다시 들이기로 하면 이 폴더에서 시작하고, 어떤 구조로 들였는지를 새 항목에 적는다.

두 구현의 비용·부하 비교는 아직 같은 오디오로 나란히 재지 않았다. 그것이 이 결정을 닫기 전에
남은 일이다.
