...
| 항목 | 값 |
|---|---|
| pytest | 489 통과, 기준 435 (`.venv/bin/python -m pytest`) |
| 새 테스트 | `tests/capture/test_recovery.py` 21, `test_recovery_loop.py` 19, `test_worker.py` 14. 50개는 고치기 전 코드에서 실패를 먼저 봤다. 3개는 바로 통과했다(기존 handoff 우회를 확인하는 수동 재시도 2개, 워커 모드 봇의 import 검사는 코드를 망가뜨려 실패하는 것을 확인했다). 재시작 안내 중복 1개는 구현 뒤 썼고 막는 코드를 빼면 실패하는 것을 확인했다 |
