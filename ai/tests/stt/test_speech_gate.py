...
# ------------------------------------------------------------------ 짧은 발화
def test_short_utterances_are_judged_with_the_lenient_threshold():
    """실로는 확신이 오르는 데 수백 ms 걸린다. 1초 미만 발화는 그 안에서 끝난다.

    실제 녹음의 "감사합니다" 0.40초가 임계 0.7 에서는 말 비율 0.97, 0.8 부터는 0.00
    이었다. 0.95 하나로 보면 짧은 대답이 전부 사라진다.
    """
    from stt.speech_gate import SHORT_SPEECH_S, SHORT_THRESHOLD, THRESHOLD

    seen = []

    def spy(pcm, sample_rate, threshold):
        seen.append(threshold)
        return all_speech(pcm, sample_rate, threshold)

    gate = SpeechGate(speech_spans=spy)
    pcm = noise(1_800)                       # 뒤 800ms 무음까지 포함한 길이
    assert gate.accepts(pcm, SR, speech_s=0.4)
    assert gate.accepts(pcm, SR, speech_s=2.0)
    assert seen == [SHORT_THRESHOLD, THRESHOLD]
    assert SHORT_THRESHOLD < THRESHOLD
    assert 0.4 < SHORT_SPEECH_S < 2.0


def test_speech_length_defaults_to_the_clip_length():
    """길이를 안 주면 pcm 길이로 판단한다. 벤치와 오프라인 재검증이 이 경로를 쓴다."""
    from stt.speech_gate import SHORT_THRESHOLD, THRESHOLD
    ...
    gate = SpeechGate(speech_spans=spy)
    gate.accepts(noise(500), SR)
    gate.accepts(noise(3_000), SR)
    assert seen == [SHORT_THRESHOLD, THRESHOLD]
