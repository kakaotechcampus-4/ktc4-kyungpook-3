...
from stt.latency import format_summary, summarize, write_latency
...
def test_average_would_hide_the_slow_call():
    ...
    assert format_summary(one_slow) != format_summary(all_middling)


def test_unmeasured_stage_says_so_instead_of_zero():
    """0 은 "즉시" 로 읽힌다. 못 잰 것은 못 쟀다고 적어야 한다."""
    out = format_summary([L(1, 0, queue_s=0.1, transcribe_s=1.0, publish_s=None)])
...
def test_no_utterances_says_so():
    assert format_summary([]) == "지연 미측정 (확정된 발화 없음)"


def test_partial_lines_are_excluded():
    lines = [L(1, 0, transcribe_s=9.0, final=False), L(2, 0, transcribe_s=1.0)]
    assert summarize(lines)["transcribe"] == {"n": 1, "median_s": 1.0, "max_s": 1.0}
...
def test_writes_one_row_per_final_line_in_transcript_order(tmp_path):
    ...
    path = write_latency(lines, tmp_path)
...
def test_unmeasured_values_are_written_as_null(tmp_path):
    ...
    path = write_latency([L(1, 0, queue_s=0.1, transcribe_s=1.0, publish_s=None)], tmp_path)
