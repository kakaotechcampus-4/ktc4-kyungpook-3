...
| 6 | `task` | 시작일이 없다 | `start_date` (nullable, `YYYY-MM-DD`) 를 응답과 `POST`·`PATCH` 에 추가 | 간트차트에 기간 축이 없다. `due_date` 만으로는 막대를 그릴 수 없다 (D-016, D-170) |
...
| 5 | 태스크의 시작일 | `start_date` 가 없거나 `null` 이면 `created_at` 의 날짜 부분을 시작일로 본다 | §4.7-6 의 답을 기다리지 않고 간트를 그릴 수 있게 한다 (D-170) |

3번이 틀리면(예: 워크스페이스마다 도메인이 다르다) 링크만 깨진다. 화면은 그대로 동작한다.
5번은 §4.7-6 이 받아들여지면 폴백으로만 남는다. 거절되면 그대로 유일한 시작일이 된다.
...
- **간트의 기간은 `start_date` ~ `due_date` 다** (D-170). `start_date` 는 §4.7-6 으로 요청 중이며, 없으면 `created_at` 의 날짜 부분으로 대체한다 (§4.8-5). 응답을 기다리지 않고 착수한다.
...
- **시작일 폴백** — `start_date` 가 없으면 `created_at` 의 날짜 부분을 쓴다. 간트가 이 값을 쓴다 (D-170).
...
- 태스크 픽스처의 `start_date` 는 **일부만 채운다.** 비어 있는 항목이 있어야 §4.8-5 의 폴백 경로가 테스트된다.
