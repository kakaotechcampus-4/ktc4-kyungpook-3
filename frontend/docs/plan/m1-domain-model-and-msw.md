...
**대체로 맞추지 않는다** — MSW 의 일은 계약을 서빙하는 것이지 서버 검증을 복제하는 것이 아니다.

**다만 한 곳은 화면이 실제로 닿는다.** 업로드의 `attendee_member_ids` 다.

```
프론트 결정(impl-decision 2026-09-18)   같은 이름 필드를 반복한다
MSW                                     form.getAll('attendee_member_ids')
백엔드                                  attendee_member_ids: str = Form(...)   ← 단일 값
```

**참석자 2명 이상 업로드는 화면이 늘 만드는 정상 요청이다.** 백엔드가 지금은 값을 버려서 드러나지 않지만,
참석자 저장을 구현하면 **일부만 저장된다.** 계약 §4.0-②-9 에 형식까지 명시해 올렸다.
통합 테스트가 한 명만 보내고 있어 이 차이가 가려져 있었다.
...
#### 의도적으로 다른 것 — 백엔드가 스텁이거나 제품 결정과 다르다
...
| 5 | 회의 업로드 | 파일·참석자를 저장하지 않고 **검증도 하지 않는다** | 날짜·참석자·Notion 연결을 검증하고 참석자 수를 보존한다 | `# TODO: 파일 저장 및 큐 전송` |
...
| 8 | 워크스페이스 이름 중복 | 원문 **완전일치 · 전역** 비교 | 공백 정규화 후 **같은 계정 안에서만** 비교 (D-015~D-020, 계약 §4.0-②-2) | 표시 없음 |
