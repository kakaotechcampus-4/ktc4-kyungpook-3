...
- 작성일: 2026-09-15 · 개정 2026-09-22
- 상태: 초안
- 근거: `frontend/docs/decision/frontend-decisions.md` (D-001~D-172)
...
1. **백엔드 API가 화면 요구를 거의 충족하지 못한다.**
   ...
   - **2026-09-22 갱신.** PR #59로 auth · workspaces · integrations · 회의 웹 경로가 생겼다. 다만 **온보딩 저장 · Discord 목록 · 회의록 본문 · 업로드가 스텁이고 OAuth 경로가 없다.** 메시지 API는 여전히 없고(D-171로 1차 제외) CORS도 없어 **Vite 프록시가 로컬 연동의 유일한 경로인 것은 그대로다.** MSW 우선은 유지된다(D-172).
...
- **계약 우선(contract-first), 단 합의 없이.** 백엔드와 API를 사전 합의하지 않는다(D-159). 프론트엔드가 필요한 요청과 응답을 **스스로 가정해 명세로 고정하고**(M1) 그 위에 화면을 올린다. 도메인 모델이 흔들리면 MSW 픽스처와 화면이 함께 재작업되므로 UI보다 먼저, 한 번에 끝낸다.
  - 다만 **결정이 있는 범위만** 확정한다. 태스크 목록의 필터·정렬·페이지네이션 파라미터와 워크스페이스 설정 항목은 제품 결정이 없으므로 계약 문서에 **`결정 대기`로 표기만** 해 백엔드가 존재를 알되 막히지는 않게 한다. 없는 결정을 계약으로 앞질러 확정하지 않는다.
  - 보드 컬럼(D-169), 간트 기간(D-170), 메시지의 1차 범위(D-171)는 계약 작성 중에 결정을 받아 확정했다. 계약 문서 §8 에 M1 을 막는 항목이 남아 있지 않다.
...
- 모든 화면은 **MSW를 유일한 데이터 출처**로 개발한다. 실제 API가 제공되면 D-133에 따라 생성 타입으로 교체하고 **entities 계층의 변환만** 수정한다.
  - **2026-09-22 — 실제 API가 도착했다.** PR #59가 계약 §4.1~§4.5를 구현했고 대조를 마쳤다(D-172). 가정 명세는 유지한다. **필드 이름이 일치해 도메인 타입도 mapper도 재작성하지 않았다.** 다른 것은 대부분 값이었다.
    형태가 다른 셋(204 무본문, 회의 목록 `title` nullable, `role` Optional)은 mapper와 API 클라이언트가 흡수한다.
  - MSW는 이제 **가정이 아니라 실 API의 의미를 흉내낸다.** 형태만 맞추면 MSW로 통과한 화면이 실 API에서 깨진다. 태스크 쓰기 경로(PATCH의 `null`, 되돌리기 충돌)를 먼저 맞췄다.
  - 온보딩 단계 저장 · Discord 사용자 목록 · 회의록 본문 · **회의 업로드** 넷은 **서버가 스텁**이라 MSW 쪽이 더 완전하다. 이 넷은 계속 MSW로 개발한다.
- 가정 명세는 회의 안건이 아니라 **백엔드에 넘길 인수인계 자료**로 유지한다. 백엔드를 향한 요청은 그 문서의 §4 한 절에 모은다(D-160).
...
- **`결정 대기`로 표기만 하는 대상** — 존재를 알리되 스펙을 확정하지 않는다:
  태스크 목록의 필터·정렬·페이지네이션 파라미터, 워크스페이스 설정 항목, 로그아웃의 동작과 이동 경로(D-165). 해당 제품 결정이 나오면 계약 문서에 추가한다.
- **보드·간트·캘린더는 전용 조회를 두지 않는다.** 목록과 같은 `GET /tasks?workspace_id=` 전량 조회를 공유한다(D-166, D-169, D-170). 간트의 시작일만 `task.start_date` 를 추가로 요청하며, 없으면 `created_at` 으로 대체한다.
- **메시지는 1차에서 만들지 않는다**(D-171). 라우트와 내비 항목만 두고 API 를 요청하지 않는다.
- 한 번 적은 요청·응답은 **화면 사정으로 임의로 바꾸지 않는다.** 바꾸면 MSW 픽스처와 통합 테스트가 함께 흔들린다.
- 백엔드 요청 목록을 이 명세의 §4 로 통합한다. 별도의 요청 문서를 두지 않는다(D-160).
...
**실 API 제약 — M4는 MSW로 완결하고 연결은 뒤로 미룬다** (D-172, 계약 §4.0).
...
**미결**: PM 외 업로드 권한, 정리 중 `회의 올리기` 진입 동작(D-090), 상단 내비게이션 `정리 중` 표시 여부, 일반 팀원의 `원본 듣기` 권한. 회의 파일 최대 용량·길이는 **프론트가 클라이언트 상수로 먼저 정하고**(D-159) 실제 서버 한도가 생기면 맞춘다. `AUDIO_TOO_LARGE`(413) 코드는 생겼지만 한도도 없고 던지는 곳도 없다(D-172).

**실 API 제약 — 업로드는 실 백엔드로 끝까지 돌지 않는다** (D-172, 계약 §4.0).

- **파일을 저장하지 않는다.** 업로드는 `processing` 회의 행만 만들고 끝난다(`# TODO: 파일 저장 및 큐 전송`). 그 회의가 영원히 `processing` 이라 **다음 업로드를 409로 막는다.** 실 백엔드로는 두 번째 업로드부터 불가능하다.
- **참석자를 받고 버린다.** D-085·D-086의 「최소 1명, 등록 팀원 중에서만」은 **프론트엔드 폼이 유일한 방어**다.
- **Notion 미연결 409를 던지지 않는다.** D-096~D-098의 차단 모달은 업로드 응답이 아니라 `GET /integrations`를 먼저 읽어 판단한다. 계약 §5.5의 진입 가드가 그 경로다.
- **`revoked`가 없다.** D-099·D-100의 끊김 모달과 D-097의 미연결 모달이 실 API로는 구분되지 않는다. 두 모달의 분기는 MSW로만 검증된다.
- **회의록 본문이 스텁이다.** `transcript`가 하드코딩 1줄, `summary`가 항상 `null`, `attendees`는 화자에서 역산이라 로컬 업로드는 빈 배열이다. 필드 이름과 형태는 계약대로라 **mapper와 화면은 그대로 쓴다.** 값만 비어 있다.
- 따라서 **M5도 MSW로 완결하고 연결은 뒤로 미룬다.** 진행률 UI가 쓰는 `progress`는 §4.7-1로 복구돼 실 API에서도 온다.
