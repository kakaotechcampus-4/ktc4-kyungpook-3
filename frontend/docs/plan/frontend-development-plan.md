...
1. **백엔드 API가 화면 요구를 거의 충족하지 못한다.**
   ...
   - **2026-09-22 갱신.** PR #59로 auth · workspaces · integrations · 회의 웹 경로가 생겼다. 다만 **온보딩 저장 · Discord 목록 · 회의록 본문이 스텁이고 OAuth 경로가 없다.** 메시지 API는 여전히 없고(D-171로 1차 제외) CORS도 없어 **Vite 프록시가 로컬 연동의 유일한 경로인 것은 그대로다.** MSW 우선은 유지된다(D-172).
...
- 모든 화면은 **MSW를 유일한 데이터 출처**로 개발한다. 실제 API가 제공되면 D-133에 따라 생성 타입으로 교체하고 **entities 계층의 변환만** 수정한다.
  - **2026-09-22 — 실제 API가 도착했다.** PR #59가 계약 §4.1~§4.5를 구현했고 대조를 마쳤다(D-172). 가정 명세는 유지한다. **필드 이름과 형태가 일치해 도메인 타입도 mapper도 재작성하지 않았다.** 다른 것은 값이었다.
  - MSW는 이제 **가정이 아니라 실 API의 의미를 흉내낸다.** 형태만 맞추면 MSW로 통과한 화면이 실 API에서 깨진다. 태스크 쓰기 경로(PATCH의 `null`, 되돌리기 충돌)를 먼저 맞췄다.
  - 온보딩 단계 저장 · Discord 사용자 목록 · 회의록 본문 셋은 **서버가 스텁**이라 MSW 쪽이 더 완전하다. 이 셋은 계속 MSW로 개발한다.
